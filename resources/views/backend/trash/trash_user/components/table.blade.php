    ...
    <tbody>
        @foreach ($data as $index => $item)
            <tr>
                ...
                <td>
                    <form action="{{ route('admin.users.restore', $item->id) }}" method="POST" style="display: inline;">
                        @csrf
                        <button type="submit" class="btn btn-success"><i class="fa fa-undo"></i></button>
                    </form>
                    @if(auth()->user()->hasPermission('forceDeleteUser'))

                    <form action="" method="POST" data-url="users" class="form-delete" style="display: inline;">
                        @method('DELETE')
                        @csrf
